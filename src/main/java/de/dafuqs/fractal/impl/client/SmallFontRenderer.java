...
public class SmallFontRenderer {
	...
	private static final Identifier SILVER_REF = Identifier.of("fractal", "textures/gui");
	private static final Identifier SILVER_FNT = Identifier.of("fractal", "textures/gui/silver.fnt");
		...
		for (int i = 0; i < codePoints.length; i++) {
			...
			if (!forceUnicode && c <= 0x7F) {
				...
				if (lastTexture != TINYFONT_TEXTURE) {
					...
					bb = tess.begin(DrawMode.QUADS, VertexFormats.POSITION_TEXTURE);
				}
	...
	private static void quad(Matrix4f mat, VertexConsumer vc, float x, float y, float u, float v, float width, float height, float texW, float texH) {
		...
		vc.vertex(mat, x1, y1, 0).texture(u1, v1);
		vc.vertex(mat, x1, y2, 0).texture(u1, v2);
		vc.vertex(mat, x2, y2, 0).texture(u2, v2);
		vc.vertex(mat, x2, y1, 0).texture(u2, v1);
	}
	...
	private static final class Reloader extends SinglePreparationResourceReloader<AngelFont> implements IdentifiableResourceReloadListener {
		...
		private static final Identifier ID = Identifier.of("fractal", "font_loader");
